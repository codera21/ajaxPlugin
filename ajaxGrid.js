      ...
      var self = {
        ...
        createTable: function(offset, pageSize, sortExpression, sortOrder, pageNumber) {
          ...
          $.ajax({
            ...
            success: function(data) {
              ...
              if (options.pagingStyle === 'default') {
                self.createPaging(data.RowCount, pageNumber, options.pageSize);
              } else if (options.pagingStyle === 'custom') {
                self.createPagingCustom(data.RowCount, pageNumber, options.pageSize);
              }
            }
          });
        },
        ...
        createPaging: function(rowCount, pageNumber, pageSize) {
          ...
          if (showLastPage == true) {
            $pageLi = $('<li class="page-item"/>');
            $pageLink = $("<a class='dots page-link'/>");
            $pageLink.text('...');
            $pageLi.append($pageLink);
            pageUl.append($pageLi);
          ...
          if (rowCount > 1) {
            ...
            var $previousPageLink = $("<a class='page-link'/>").text('Previous').addClass('previous');
          ...
          if (rowCount > 1) {
            ...
            var $nextPageLink = $("<a class='page-link'/>").text('Next').addClass('next');
        ...
        /**************************/
        createPagingCustom: function(rowCount, pageNumber, pageSize) {

          pageUl = $('<ul/>');
          var pageRange = 1; // page is  1 by default
          var pagesToShow = 5;
          var prvPostPages = 3;
          if (rowCount > options.pageSize)
            pageRange = Math.ceil(rowCount / options.pageSize);
          ...
          if (pageRange > pagesToShow) {
            if (pageNumber < pagesToShow) {
              ...
              pageNumberShowLower = parseInt(pageNumber) - prvPostPages;
              pageNumberShowUpper = parseInt(pageNumber) + prvPostPages;
              showFirstPage = true;
              showLastPage = true;
            }
          } else {
          ...
          if (showLastPage == true) {
            $pageLi = $('<li/>');
            $pageLink = $("<a />");
            if (options.pagingStyle.dots != null) {
              $.each(options.pagingStyle.dots, (key, item) => {
                $pageLink.addClass(item);
              });
            }
            $pageLink.html('&hellip;');
            $pageLi.append($pageLink);
            pageUl.append($pageLi);
          ...
          if (rowCount > 1) {
            ...
            if (pageNumber == 1) {
              $previousLi.addClass('disabled');
              $previousPageLink.attr('disabled', true);
            }
          ...
          if (rowCount > 1) {
            ...
            if (pageNumber == pageNumberShowUpper) {
              $nextLi.addClass('disabled');
              $nextPageLink.attr('disabled', true);
            }
          ...
          if (options.pagingClasses != null) {
            let ulClasses = options.pagingClasses.ul;
            let liClasses = options.pagingClasses.li;
            let aClasses = options.pagingClasses.a;

            if (ulClasses != null) {
              $.each(ulClasses, (key, item) => {
                pageUl.addClass(item);
              });
            }
            if (liClasses != null) {
              $.each(liClasses, (key, item) => {
                pageUl.find('li').addClass(item);
              });
            }
            if (aClasses != null) {
              $.each(aClasses, (key, item) => {
                pageUl.find('a').addClass(item);
              });
            }
          }
